...
responsders.enrichment = nonresponsders.enrichment = list()
...
for (ii in seq(ncol(clust.mat) -1)) {
    ...
    Idents(sco.sub) = ifelse(sco.sub$seurat_clusters %in% seurat.clust.cell.1, 0 ,1) %>% as.factor
    ...
    markers.top  = markers.curr %>% group_by(cluster) %>% top_n(n = 50, wt = avg_logFC)
    term2gene  = markers.top %>%  as.data.table %>% 
    .[,.(term=cluster,gene=gene)]
    ## perform enrichment see : https://yulab-smu.github.io/clusterProfiler-book/chapter3.html
    responsders.enrichment[[ii]] = enricher(geneList=deg.curr[log2FoldChange> 0]$gene, term2gene = term2gene, pvalueCutoff=1) 
    nonresponders.enrichment[[ii]] = enricher(geneList=deg.curr[log2FoldChange< 0]$gene, term2gene = term2gene, pvalueCutoff=1) 
