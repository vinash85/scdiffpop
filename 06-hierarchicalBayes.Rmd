...
for(i in 1:nrow(Tree)) {
  ...
  res_blood$sim_vals <- as.data.frame(res_blood$sim_vals)
  res_tumor$sim_vals <- as.data.frame(res_tumor$sim_vals)
  
  Pop_diff <- 0
  Pop_diff <- sum(sapply(c(1:1000), function(x) {
    if(rbeta(1,res_blood$sim_vals$alpha[x],res_blood$sim_vals$beta[x]) > rbeta(1,res_tumor$sim_vals$alpha[x],res_tumor$sim_vals$beta[x])) {return(1)}
    else {return(0)}
  }))/1000
  ...
  graphics::boxplot(cbind(res_blood$theta_sim_vals, res_tumor$theta_sim_vals), col = c(rep("red", 4), rep("blue", 4)), main = i, ylab = "Theta", xlab = paste("Wilcox: ", wp, "TumorGap: ", TumorGap, "BloodGap:", BloodGap, "PopDiff: ", Pop_diff))
#paste("/Users/phillipnicol/Desktop/local_files/data/scDiffPopDATA/gse145281_tumor/Bayesian_model/", i, ".png", sep = "")
  
  # Now without hierarichal component
  res_blood <- list()
  res_tumor <- list()
  theta_sim_vals <- matrix(0, nrow = 1000, ncol = 4)
  theta_sim_vals[,1] <- rbeta(1000, 1+df_blood[1,1], 1+df_blood[1,2]-df_blood[1,1])
  theta_sim_vals[,2] <- rbeta(1000, 1+df_blood[2,1], 1+df_blood[2,2]-df_blood[2,1])
  theta_sim_vals[,3] <- rbeta(1000, 1+df_blood[3,1], 1+df_blood[3,2]-df_blood[3,1])
  theta_sim_vals[,4] <- rbeta(1000, 1+df_blood[4,1], 1+df_blood[4,2]-df_blood[4,1])
  res_blood$theta_sim_vals <- theta_sim_vals
  
  theta_sim_vals <- matrix(0, nrow = 1000, ncol = 4)
  theta_sim_vals[,1] <- rbeta(1000, 1+df_tumor[1,1], 1+df_tumor[1,2]-df_tumor[1,1])
  theta_sim_vals[,2] <- rbeta(1000, 1+df_tumor[2,1], 1+df_tumor[2,2]-df_tumor[2,1])
  theta_sim_vals[,3] <- rbeta(1000, 1+df_tumor[3,1], 1+df_tumor[3,2]-df_tumor[3,1])
  theta_sim_vals[,4] <- rbeta(1000, 1+df_tumor[4,1], 1+df_tumor[4,2]-df_tumor[4,1])
  res_tumor$theta_sim_vals <- theta_sim_vals
